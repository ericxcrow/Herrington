const { Modal, TextInputComponent, MessageEmbed, Permissions, MessageAttachment, MessageActionRow, MessageButton, MessageSelectMenu } = require('discord.js');
...
module.exports = async (bot, interaction) => {
    ...
    if(interaction.values == `mutes_roles`){
      ...
      if(!guilds.moderation.get('isModerating')) return interaction.reply({ content: `Ошибка! Система модерации выключена на данном сервере.`, ephemeral: true })
      if(guilds.moderation.get('mutes')) return interaction.reply({ content: `Ошибка! Система выдачи роли мута уже включена на данном сервере.`, ephemeral: true })
      ...
      guilds.set(`moderation.mutes`, true)
    ...
    if(interaction.values == `mutes_timeot`){
      if(!interaction.guild.me.permissions.has("MANAGE_ROLES") & !interaction.guild.me.permissions.has("MODERATE_MEMBERS")) return interaction.reply({ content: `Ошибка! У меня нет прав на управление ролями/выдачу тайм-аутов. Рекомендуется дать боту права "Администратора"...`, ephemeral: true })
      if(!guilds.moderation.get('isModerating')) return interaction.reply({ content: `Ошибка! Система модерации выключена на данном сервере.`, ephemeral: true })
      if(!guilds.moderation.get('mutes')) return interaction.reply({ content: `Ошибка! Система выдачи тайм-аутов уже включена на данном сервере.`, ephemeral: true })

      await interaction.guild.roles.delete(`${guilds.get(`moderation.mute_role`)}`, `Удаление роли мута`).catch(() => null)

      guilds.set(`moderation.mutes`, false)
